...
void Traffic::setadd(char *a1, char *a2) { strcpy_s(add1, 15, a1); strcpy_s(add2, 15, a2); }
void Traffic::setdst(double d) { distance = d; }
istream & operator>>(istream &input, Traffic & R)  //重载提取运算符，输入项目信息
{
...
void Traffic::print()
{
	...
	cout << "公共交通工程" << "\t" << code << "\t" << name << "\t" << (status == true ? "已审批" : "未审批") << "\t建设单位：" << company << "\t建筑地址：" << address.city << address.district << address.street << "\n\t施工许可面积：" << area << "平方米\t合同金额：" << cost << "万元人民币\t资金来源：";
	if (source == 0) cout << "项目公司";
	if (source == 1) cout << "社会捐赠";
	if (source == 2) cout << "两者均有";
	if (source) cout << "\t社会捐赠金额：" << donation;
	cout << "\n\t合同开工日期：" << begin.year << "年" << begin.month << "月" << begin.day << "日\t合同完工日期：" << end.year << "年" << end.month << "月" << end.day << "日";
	cout << "\n\t路线起点：" << add1 << "\t路线终点：" << add2 << "\t路线距离：" << distance << "米" << endl;
}