...
istream & operator>>(istream &input, Office & R)  //重载提取运算符，输入项目信息
{
...
void Office::print()
{
	...
	cout << "办公建筑" << "\t" << code << "\t" << name << "\t" << (status == true ? "已审批" : "未审批") << "\t建设单位：" << company << "\t建筑地址：" << address.city << address.district << address.street << "\n\t施工许可面积：" << area << "平方米\t合同金额：" << cost << "万元人民币\t资金来源：";
